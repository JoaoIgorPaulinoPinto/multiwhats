...
export const PARAM_HINTS: Record<string, string> = {
  ...
  "Auth:RequireRegistrationCode": "Quando ligado, novos usuários precisam de um código de permissão gerado na tela de configurações.",
  "Auth:RegistrationCodeExpiryHours": "Depois desse tempo o código expira e não pode mais ser usado.",
  "Business:Enabled": "Quando ligado, mensagens fora do horário recebem uma resposta automática.",
  "Business:OpenTime": "Ex: 08:00",
  "Business:CloseTime": "Ex: 18:00",
  "Business:WorkingDays": "Selecione os dias da semana em que o atendimento está disponível.",
  "Business:OutsideHoursMessage": "Use {open}, {close} e {days} para preencher automaticamente.",
...
export function serializeValue(param: SystemParameterResponse, display: string): string {
  if (param.type === "JsonList") {
    const arr = display
      .split(",")
      .map((s) => s.trim())
...
export function validateValue(param: SystemParameterResponse, display: string): string | null {
  if (param.type === "Int" && !/^-?\d+$/.test(display.trim())) {
    return "Informe um número inteiro"
  }
  if ((param.key === "Business:OpenTime" || param.key === "Business:CloseTime") && !/^\d{2}:\d{2}$/.test(display.trim())) {
...
export function useSettings() {
  ...
  useEffect(() => {
    load().catch((e) => console.error("[Settings] erro ao carregar:", e)).finally(() => setLoading(false))
  }, [load])
  ...
  async function save() {
    ...
    try {
      for (const key of keys) {
        ...
        if (!param) continue
        await settingsService.update(key, serializeValue(param, values[key] ?? ""))
      }
  ...
  async function generateCode() {
    ...
    try {
      ...
      toast.error("Erro ao gerar código — apenas Administrador/Dev pode fazer isso.")
    } finally {
