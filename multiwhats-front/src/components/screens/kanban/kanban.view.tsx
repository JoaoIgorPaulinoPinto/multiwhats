...
import { ChevronLeft, ChevronRight, Plus, X, AlertCircle, Clock, User } from "lucide-react"
import { useState, useEffect } from "react"
import { useKanban, type KanbanCard } from "./kanban.logic"
import { kanbanService, type OccurrenceMetricsResponse } from "../../../services/kanban.service"
import { OccurrenceDetailModal } from "./occurrence-detail-modal"
import { CreateOccurrenceModal } from "./create-occurrence-modal"
import { OCCURRENCE_STATUS_OPTIONS, PRIORITY_COLORS, PRIORITY_LABELS, OCCURRENCE_STATUS_COLORS, OCCURRENCE_STATUS_LABELS } from "../../../constants"
...
function CardActions({ card, onAdvance }: { card: KanbanCard; onAdvance: (id: number, dir: "Advance" | "Return") => void }) {
    ...
    <div className={styles.cardActions}>
      {opt?.prev && (
        <button
          ...
          onClick={(e) => { e.stopPropagation(); onAdvance(card.id, "Return") }}
          title={opt.prevLabel}
        >
          <ChevronLeft size={14} />
        </button>
      ...
      {opt?.next && (
        <button
          className={styles.actionBtn}
          onClick={(e) => { e.stopPropagation(); onAdvance(card.id, "Advance") }}
          title={opt.nextLabel}
        >
...
function OccurrenceCard({ card, onClick, onAdvance }: { card: KanbanCard; onClick: () => void; onAdvance: (id: number, dir: "Advance" | "Return") => void }) {
  return (
    <div
    ...
    >
      <div className={styles.cardHeader}>
        ...
        <span
          ...
          style={{ background: PRIORITY_COLORS[card.priority as keyof typeof PRIORITY_COLORS] ?? "#6b7280" }}
          title={PRIORITY_LABELS[card.priority as keyof typeof PRIORITY_LABELS] ?? String(card.priority)}
        />
      </div>

      {card.description && (
        <p className={styles.cardDescription}>{card.description.length > 80 ? card.description.slice(0, 80) + "…" : card.description}</p>
      )}
...
function ClosedMetricsModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    kanbanService.getOccurrenceMetrics()
      .then(setMetrics)
      .catch(console.error)
    ...
    <>
      ...
      <div className={`${styles.modal} ${styles.metricsModal}`}>
        ...
        {loading ? (
          ...
          <div className={styles.metricsContent}>
            <div className={styles.metricsGrid}>
              <div className={styles.metricCard}>
                <strong className={styles.metricValue}>{metrics.totalClosed}</strong>
                <span className={styles.metricLabel}>Total fechadas</span>
              </div>
              <div className={styles.metricCard}>
                <strong className={styles.metricValue}>{metrics.averageResolutionHours}h</strong>
          ...
          <p style={{ color: "var(--text-muted)", textAlign: "center", padding: 20 }}>Erro ao carregar métricas.</p>
        )}
      </div>
    </>
  )
...
function ClosedOccurrencesModal({ occurrences, onAdvance, onCardClick, onClose }: {
      ...
      <div className={`${styles.modal} ${styles.closedModal}`}>
        ...
        <div className={styles.closedList}>
          {occurrences.map((card) => (
            <div key={card.id} className={styles.closedItem} onClick={() => onCardClick(card)}>
              <div className={styles.closedItemInfo}>
                <strong>{card.title}</strong>
                <span>{card.description ? (card.description.length > 60 ? card.description.slice(0, 60) + "…" : card.description) : "Sem descrição"}</span>
                <small>{formatDate(card.createdAt)} - {card.assignedToName ?? "Sem responsável"}</small>
              </div>
              <CardActions card={card} onAdvance={onAdvance} />
            </div>
          ))}
...
export function KanbanView() {
  const { columns, loading, load, advanceStatus, deleteOccurrence, occurrences } = useKanban()
      ...
      <header className={styles.header}>
        ...
        <div className={styles.headerActions}>
          {closedCards.length > 0 && (
            <button className={styles.metricsBtn} onClick={() => setShowMetrics(true)}>
              <AlertCircle size={14} />
              Métricas
            </button>
          )}
            ...
            return (
              <div key={column.id} className={styles.column}>
                ...
                {isClosed && hasMoreClosed && (
                  <button className={styles.viewAllBtn} onClick={() => setShowClosedModal(true)}>
