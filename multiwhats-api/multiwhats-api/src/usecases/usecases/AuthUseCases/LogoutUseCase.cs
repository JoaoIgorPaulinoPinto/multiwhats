...
public class LogoutUseCase : ILogoutUseCase
{
    ...
    public LogoutUseCase(TokenBlacklistService blacklist, TokenService tokenService, UseCaseLogger useCaseLogger)
    {
    ...
    public async Task Execute(ClaimsPrincipal user, string token, string? refreshToken = null)
    {
        ...
        {
            var refreshJti = _tokenService.DecodeJti(refreshToken);
            if (refreshJti != null)
            {
                _blacklist.Revoke(refreshJti, DateTime.UtcNow.AddDays(30));
            }
        }
